...
ALPHA_REGISTRY: tuple[AlphaSpec, ...] = (
    ...
    AlphaSpec(
        name="alpha_residual_momentum_21d",
        category="residual_price_path",
        formula="sum(ret_1d - market_return_1d, 21)",
        source="local_formula",
        expected_direction=1,
        lookback=21,
        description="Twenty-one day momentum after removing the equal-weight market move.",
    ),
    AlphaSpec(
        name="alpha_residual_reversal_5d",
        category="residual_price_path",
        formula="-sum(ret_1d - market_return_1d, 5)",
        source="local_formula",
        expected_direction=1,
        lookback=5,
        description="Five-day reversal after removing the equal-weight market move.",
    ),
    AlphaSpec(
        name="alpha_range_compression_21d",
        category="price_path",
        formula="-((high - low) / mean(high - low, 21) - 1)",
        source="local_formula",
        expected_direction=1,
        lookback=21,
        description="Daily range compression versus the recent average range.",
    ),
    AlphaSpec(
        name="alpha_volatility_contraction_21d",
        category="risk",
        formula="-(std(ret_1d, 5) / std(ret_1d, 21) - 1)",
        source="local_formula",
        expected_direction=1,
        lookback=21,
        description="Short-term volatility contraction relative to the one-month baseline.",
    ),
    AlphaSpec(
        name="alpha_volume_trend_5_21d",
        category="volume",
        formula="mean(volume, 5) / mean(volume, 21) - 1",
        source="local_formula",
        expected_direction=1,
        lookback=21,
        description="Short-term volume acceleration versus the one-month baseline.",
    ),
    AlphaSpec(
        name="alpha_intraday_quality_21d",
        category="intraday_position",
        formula="mean((close - open) / open, 21) / std((close - open) / open, 21)",
        source="local_formula",
        expected_direction=1,
        lookback=21,
        description="Recent consistency of positive intraday buying pressure.",
    ),
    AlphaSpec(
        name="alpha_gap_pressure_21d",
        category="overnight_path",
        formula="mean(open / delay(close, 1) - 1, 21) / std(open / delay(close, 1) - 1, 21)",
        source="local_formula",
        expected_direction=1,
        lookback=21,
        description="Recent consistency of positive overnight gap pressure.",
    ),
    AlphaSpec(
        name="ml_prediction_5d",
        category="machine_learning",
        formula="walk_forward_ml_model(factors -> future_return_5d)",
        source="local_ml",
        expected_direction=1,
        lookback=252,
        description="Out-of-sample five-day return prediction from a walk-forward ML model.",
    ),
)
