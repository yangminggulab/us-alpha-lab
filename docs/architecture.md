...
7. alpha-lab train
   lagged factors -> future returns -> model metrics + data/models/latest_model.joblib
...
- 因子先透明后复杂：先用价格、成交量、VWAP 的可解释因子，再引入更多公式因子。
...
- `modeling.py`: 用滞后后的因子预测未来 5 日收益，训练前把 `inf` 这类异常值转成缺失值，再交给中位数填补器。
